...
def test_scamp_linear(config, scamp_data_path, torch_data_path, debug):
    ...
    final_conv_method = quantization.get_method_func(config['train_params']['final_conv_method'])
    linear_method = quantization.get_method_func(config['train_params']['linear_method'])
    # print("cnn method ",cnn_method)
    ...
    #         scamp_test_ids.append(vid_path)
    #         scamp_test_labels.append(int(cls))


    # torch_ds = dataset_classes.ScampDataset(ids= torch_test_ids, labels= torch_test_labels, input_white=2.0, pytorch_size=True)
    # torch_dl = DataLoader(torch_ds, batch_size=1, num_workers=1, shuffle=False) 

    # scamp_ds = dataset_classes.ScampDataset(ids= scamp_test_ids, labels= scamp_test_labels, input_white=5.0)
    # scamp_dl = DataLoader(scamp_ds, batch_size=1, num_workers=1, shuffle=False) 
    ...
    fine_tune_data_path = '/media/data4b/haleyso/cambridge_all_outputs_batched/'
    train_2_data_path = '/media/data4b/haleyso/cambridge_all_outputs_batched_2/'

    
    classes = os.listdir(fine_tune_data_path)
    classes.sort()
    for cls in range(len(classes)): 
        class_path = os.path.join(fine_tune_data_path, str(cls))
        videos = os.listdir(class_path)
        videos.sort()
        # print(len(videos))
        for vid in videos:
            ...
            if os.path.exists(vid_path):
                if vid in train_set_names:
                    fine_tune_ids.append(vid_path)
                    fine_tune_labels.append(int(cls))
                    val_ids.append(os.path.join(train_2_data_path, str(cls), vid, "15_output.BMP" ))
                    val_labels.append(int(cls))
                else:
                    test_ids.append(vid_path)
                    test_labels.append(int(cls))

    scamp_ds = dataset_classes.ScampDataset(ids= test_ids, labels= test_labels, input_white=5.0)
    scamp_dl = DataLoader(scamp_ds, batch_size=1, num_workers=1, shuffle=False) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-td', '--torch_data', default='/home/haleyso/CNN_CGRU/intermediates/', type=str, help='intermediates torch path (default: {default}})')
    parser.add_argument('-d', '--debug', default=False, type=str, help='True: use pytorch inputs. False: use SCAMP inputs')
    ...
    test_scamp_linear(config, args.scamp_data, args.torch_data, args.debug)
